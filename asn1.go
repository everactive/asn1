...
func parseField(v reflect.Value, bytes []byte, initOffset int, params fieldParameters) (offset int, err error) {
	...
	if !params.explicit && params.class != nil && params.tag != nil {
		expectedClass = *params.class
		expectedTag = *params.tag
		matchAnyClassAndTag = false
	}
	...
	if !matchAnyClassAndTag && (t.class != expectedClass || t.tag != expectedTag) ||
		...
		if ok {
			...
			err = StructuralError{fmt.Sprintf("tags don't match ({expectedClass: %d expectedTag:%d} vs %+v) %+v %s @%d", expectedClass, expectedTag, t, params, fieldType.Name(), offset)}
		}
		return
	}
	if invalidLength(offset, t.length, len(bytes)) {
	...
	if params.littleEndian {
		innerBytes = reverseByteArray(innerBytes)
	}
